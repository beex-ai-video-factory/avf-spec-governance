# R15 END-TO-END INTEGRATION & SCENARIO TEST HARNESS — ACCEPTANCE & RELEASE
## AI Video Factory — Formal Acceptance, Merge & Version Tagging

**PROMPT_ID:** `R15-04`  
**PURPOSE:** Conduct formal acceptance signoff for R15_integration_harness (End-to-End Integration & Scenario Test Harness), merge feature branch to main, apply annotated git release tag, and unlock downstream dependency gates.  
...
**COMMAND_TO_RUN:**
```bash
/goal Read 04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/06_R15_INTEGRATION_HARNESS/R15_04_ACCEPT_RELEASE.md and execute it completely.
```  
**EXPECTED_ARTIFACTS:**
...
- Clean merge commit and git tag `r15_integration_harness-v1.0.0` applied.
...
**NEXT_PROMPT_IF_PASS:** `04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/17_INTEGRATION_GATES/GATE_01_FAKEPROVIDER_E2E_GATE.md`  
...
1. **Verify Audit Signoff:**
   Inspect `05_IMPLEMENTATION/repos/R15_integration_harness/AUDIT_REPORT.md` and confirm PASS verdict.
2. **Merge to Main Branch:**
   Checkout `main` and merge `feature/r15-impl` cleanly:
   `git checkout main && git merge --no-ff feature/r15-impl -m "feat(r15): complete R15_integration_harness implementation"`
3. **Apply Release Tag:**
   Apply annotated git tag:
   `git tag -a "r15_integration_harness-v1.0.0" -m "Release R15_integration_harness v1.0.0"`
4. **Update System Runtime State:**
   In `05_IMPLEMENTATION/operator-state/RUN_STATE.yaml`, set `R15_integration_harness.status: "RELEASED"` and record commit SHA and tag.
...
BLOCKERS: []
ARTIFACTS_CREATED:
  - "05_IMPLEMENTATION/repos/R15_integration_harness/ (tagged r15_integration_harness-v1.0.0)"
  - "05_IMPLEMENTATION/operator-state/RUN_STATE.yaml"
RECOMMENDED_NEXT_PROMPT: "04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/17_INTEGRATION_GATES/GATE_01_FAKEPROVIDER_E2E_GATE.md"
