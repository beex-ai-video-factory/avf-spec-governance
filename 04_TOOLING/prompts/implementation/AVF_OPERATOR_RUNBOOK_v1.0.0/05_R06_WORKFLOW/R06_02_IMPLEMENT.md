# R06 TEMPORAL WORKFLOW ORCHESTRATOR — IMPLEMENTATION & BUILD
## AI Video Factory — Source Code Authoring & Test Suite Implementation

**PROMPT_ID:** `R06-02`  
**PURPOSE:** Implement all production source code, build toolchains, unit tests, and contract fixtures for R06_workflow (Temporal Workflow Orchestrator) according to the approved PLAN.md.  
...
**COMMAND_TO_RUN:**
```bash
/goal Read 04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/05_R06_WORKFLOW/R06_02_IMPLEMENT.md and execute it completely.
```  
**EXPECTED_ARTIFACTS:**
- Production source code in `05_IMPLEMENTATION/repos/R06_workflow/src/`
- Test suites in `05_IMPLEMENTATION/repos/R06_workflow/tests/`
- Build script passing cleanly (`npm test` / `pytest`).  
**PASS_CRITERIA:**
- All components in PLAN.md fully implemented without placeholder stubs.
- OWNS boundaries strictly enforced: Temporal workflow definitions (SingleShot, MultiShot, RenderPipeline), retry policy engine, activity handlers, reconciliation logic.
- Zero forbidden dependencies imported: R09, R10, Direct DB.
- Unit and contract tests execute and pass 100%.  
**FAIL_CRITERIA:**
- Build compilation failure, test failures, or cross-repo file modifications.  
**GIT_EXPECTATION:** Clean commits on feature branch `feature/r06-impl`.  
...
**NEXT_PROMPT_IF_PASS:** `04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/05_R06_WORKFLOW/R06_03_TEST_AND_REVIEW.md`  
...
4. **Git Commit:**
   Commit all changes to `feature/r06-impl`.
...
BLOCKERS: []
ARTIFACTS_CREATED:
  - "05_IMPLEMENTATION/repos/R06_workflow/src/"
  - "05_IMPLEMENTATION/repos/R06_workflow/tests/"
RECOMMENDED_NEXT_PROMPT: "04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/05_R06_WORKFLOW/R06_03_TEST_AND_REVIEW.md"
RECOMMENDED_NEXT_TASK: "Execute independent technical and adversarial review of R06_workflow."
