# R13 HUMAN-IN-THE-LOOP OPERATOR CONSOLE — ACCEPTANCE & RELEASE
## AI Video Factory — Formal Acceptance, Merge & Version Tagging

**PROMPT_ID:** `R13-04`  
**PURPOSE:** Conduct formal acceptance signoff for R13_operator_console (Human-in-the-Loop Operator Console), merge feature branch to main, apply annotated git release tag, and unlock downstream dependency gates.  
...
**READ_ONLY_INPUTS:**
- `05_IMPLEMENTATION/repos/R13_operator_console/AUDIT_REPORT.md`
...
**COMMAND_TO_RUN:**
```bash
/goal Read 04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/16_R13_OPERATOR_CONSOLE/R13_04_ACCEPT_RELEASE.md and execute it completely.
```  
**EXPECTED_ARTIFACTS:**
...
- Clean merge commit and git tag `r13_operator_console-v1.0.0` applied.
...
**NEXT_PROMPT_IF_PASS:** `04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/17_INTEGRATION_GATES/GATE_04_SYSTEM_INTEGRATION_GATE.md`  
...
1. **Verify Audit Signoff:**
   Inspect `05_IMPLEMENTATION/repos/R13_operator_console/AUDIT_REPORT.md` and confirm PASS verdict.
2. **Merge to Main Branch:**
   Checkout `main` and merge `feature/r13-impl` cleanly:
   `git checkout main && git merge --no-ff feature/r13-impl -m "feat(r13): complete R13_operator_console implementation"`
3. **Apply Release Tag:**
   Apply annotated git tag:
   `git tag -a "r13_operator_console-v1.0.0" -m "Release R13_operator_console v1.0.0"`
4. **Update System Runtime State:**
   In `05_IMPLEMENTATION/operator-state/RUN_STATE.yaml`, set `R13_operator_console.status: "RELEASED"` and record commit SHA and tag.
...
BLOCKERS: []
ARTIFACTS_CREATED:
  - "05_IMPLEMENTATION/repos/R13_operator_console/ (tagged r13_operator_console-v1.0.0)"
  - "05_IMPLEMENTATION/operator-state/RUN_STATE.yaml"
RECOMMENDED_NEXT_PROMPT: "04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/17_INTEGRATION_GATES/GATE_04_SYSTEM_INTEGRATION_GATE.md"
