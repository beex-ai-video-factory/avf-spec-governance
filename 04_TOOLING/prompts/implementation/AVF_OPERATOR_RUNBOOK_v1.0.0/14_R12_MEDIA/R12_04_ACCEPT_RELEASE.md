# R12 MEDIA PROCESSING & ASSEMBLY SERVICE — ACCEPTANCE & RELEASE
## AI Video Factory — Formal Acceptance, Merge & Version Tagging

**PROMPT_ID:** `R12-04`  
**PURPOSE:** Conduct formal acceptance signoff for R12_media (Media Processing & Assembly Service), merge feature branch to main, apply annotated git release tag, and unlock downstream dependency gates.  
...
**COMMAND_TO_RUN:**
```bash
/goal Read 04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/14_R12_MEDIA/R12_04_ACCEPT_RELEASE.md and execute it completely.
```  
**EXPECTED_ARTIFACTS:**
...
- Clean merge commit and git tag `r12_media-v1.0.0` applied.
...
**NEXT_PROMPT_IF_PASS:** `04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/17_INTEGRATION_GATES/GATE_03_CREATIVE_MEDIA_GATE.md`  
...
1. **Verify Audit Signoff:**
   Inspect `05_IMPLEMENTATION/repos/R12_media/AUDIT_REPORT.md` and confirm PASS verdict.
2. **Merge to Main Branch:**
   Checkout `main` and merge `feature/r12-impl` cleanly:
   `git checkout main && git merge --no-ff feature/r12-impl -m "feat(r12): complete R12_media implementation"`
3. **Apply Release Tag:**
   Apply annotated git tag:
   `git tag -a "r12_media-v1.0.0" -m "Release R12_media v1.0.0"`
4. **Update System Runtime State:**
   In `05_IMPLEMENTATION/operator-state/RUN_STATE.yaml`, set `R12_media.status: "RELEASED"` and record commit SHA and tag.
...
BLOCKERS: []
ARTIFACTS_CREATED:
  - "05_IMPLEMENTATION/repos/R12_media/ (tagged r12_media-v1.0.0)"
  - "05_IMPLEMENTATION/operator-state/RUN_STATE.yaml"
RECOMMENDED_NEXT_PROMPT: "04_TOOLING/prompts/implementation/AVF_OPERATOR_RUNBOOK_v1.0.0/17_INTEGRATION_GATES/GATE_03_CREATIVE_MEDIA_GATE.md"
